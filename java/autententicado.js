...
onAuthStateChanged(auth, (user) => {
  ...
  // Manejar envío de formulario "Nuevo Manga"
  ...
  formNuevoManga.addEventListener('submit', async (e) => {
    ...
    // Preparar subida a Cloudinary
    const formData = new FormData();
    formData.append('file', portadaFile);
    formData.append('upload_preset', 'para subir mangas'); // <- Cambia esto por tu preset real
    formData.append('folder', folderPath);

    try {
      const response = await fetch('https://api.cloudinary.com/v1_1/djxnb3qrn/image/upload', { // <- Cambia TU_CLOUD_NAME
        method: 'POST',
        body: formData
      });
      ...
      // Construir objeto manga para Firebase
      const mangaData = {
        portada: urlPortada,
        sinopsis,
        autor,
      ...
      alert('Error al subir la imagen o guardar manga: ' + error.message);
    }
  });
});
